// v6 - clean + PDF attachment (raw PDF, no npm)
...
// Uses raw PDF syntax -- text only, fixed-width font
// ---------------------------------------------------------------------------
function buildPDF(date,sales,purchases,refunds){
  ...
  // Build stream content -- BT/ET text block
  const fontSize=10;
  const lineH=14;
  const startY=780;
...
async function main(){
  ...
  const pdfB64=buildPDF(date,sales,purchases,refunds);
