...
async function getData(date){
  ...
  const xs='id,quantity,price_correction,exchange_date,note';
  const[sales,purchases,refunds,expenses,exchanges]=await Promise.all([
    ...
    fetch4date('exchanges',xs,'exchange_date',date).catch(()=>[]),
  ]);
  return{sales,purchases,refunds,expenses,exchanges};
}

// ---------------------------------------------------------------------------
// Build a minimal valid PDF as a Buffer (no npm required)
// ---------------------------------------------------------------------------
function buildPDF(date,sales,purchases,refunds,expenses,exchanges){
  ...
  const corr = exchanges.reduce((s,r)=>s+Number(r.price_correction||0),0);
  const net  = rev - cost - ref - exp + corr;
  ...
  line('  Corrections: '+(corr>=0?'+':'')+fmt(corr));
...
// HTML email body
// ---------------------------------------------------------------------------
function buildHTML(date,sales,purchases,refunds,expenses,exchanges){
  const rev  = sales.reduce((s,r)=>s+Number(r.quantity)*Number(r.sell_price),0);
  const cost = purchases.reduce((s,r)=>s+Number(r.quantity)*Number(r.cost_price),0);
  const ref  = refunds.reduce((s,r)=>s+Number(r.quantity)*Number(r.refund_price),0);
  const exp  = expenses.reduce((s,r)=>s+Number(r.amount),0);
  ...
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body style="margin:0;padding:0;background:#f8fafc;font-family:sans-serif;">'
    ...
    +(corr!==0?'<td style="padding:10px;background:#f0fdf4;border-radius:6px;text-align:center;"><div style="font-size:10px;text-transform:uppercase;color:#16a34a;">Exchange Corrections</div><div style="font-size:20px;font-weight:700;color:#16a34a;">'+(corr>=0?'+':'')+fmt(corr)+'</div></td>':'')
...
async function main(){
  ...
  const{sales,purchases,refunds,expenses,exchanges}=await getData(date);
  const rev =sales.reduce((s,r)=>s+Number(r.quantity)*Number(r.sell_price),0);
  const cost=purchases.reduce((s,r)=>s+Number(r.quantity)*Number(r.cost_price),0);
  const exp =expenses.reduce((s,r)=>s+Number(r.amount),0);
  const corr=exchanges.reduce((s,r)=>s+Number(r.price_correction||0),0);
  ...
  console.log('Exchanges: '+exchanges.length+' (corrections: '+fmt(corr)+')');
  const html=buildHTML(date,sales,purchases,refunds,expenses,exchanges);
  const pdfB64=buildPDF(date,sales,purchases,refunds,expenses,exchanges);
